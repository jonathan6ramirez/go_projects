...
func TestHastLiterals(t *testing.T) {
	tests := []vmTestCase{
		{
...
func testStringObject(expected string, actual object.Object) error {
	// This line is "extracting" the value from what we actually got
	result, ok := actual.(*object.String)

	// checking to see if any errors "extracting" were thrown
	if !ok {
		return fmt.Errorf("object is not a String. got=%T, (%+v)",
			actual, actual)
	}

	// check to see if the value that we "extracted" matches the one
...
func testIntegerObject(expected int64, actual object.Object) error {
	...
	if result.Value != expected {
		return fmt.Errorf("object had wrong value.\ngot=%d, want=%d",
			result.Value, expected)
	}
