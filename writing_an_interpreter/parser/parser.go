...
// INFO: adds entry to the maps with the associated parsing function -- next 2 functions
...
func (p *Parser) parsePrefixExpression() ast.Expression {
	...
	// move the current token to the next token to read
	p.nextToken()

	// prefix here being a const value of 6
	// and assign the the right for the expression
...
func (p *Parser) parseLetStatement() *ast.LetStatement {
	// Create the let statement node with the current token
	stmt := &ast.LetStatement{Token: p.curToken}

	// check to see if the next token is an identifier token
	...
	//check to see if the next token is an assignment token
	// if not throw an error
